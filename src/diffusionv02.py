        ...
        while t < endT:
            ...
            for k in range(len(p2De)):           
                ...
                if p2De[k-1,5]>bandgap:
                    r=float(mfp/(eemfp(p2De[k-1,5]-bandgap)/10))
                     
                    #print int(r*100)
                    if r<=1:
                        ranlist=[0]*(100-int(r*100))+[1]*int(r*100)
                        #p2De[k-1,5]=p2De[k-1,5]-random.uniform(bandgap,p2De[k-1,5])*random.choice(ranlist) #e-e scattering
                        p2De[k-1,5]=p2De[k-1,5]-1.2*random.uniform(0,1.2)*random.choice(ranlist) #e-h scattering
                     
                    else:
                        #p2De[k-1,5]=p2De[k-1,5]-np.minimum(int(r),int(p2De[k-1,5]/bandgap))*random.uniform(bandgap,p2De[k-1,5])#e-e scattering *int(mfp/(eemfp(p2De[k-1,5]-bandgap)/10))
                        p2De[k-1,5]=p2De[k-1,5]-np.minimum(int(r),int(p2De[k-1,5]/bandgap))*random.uniform(0,1.2)#e-h scattering
...
def scanplot(wlscan,start,stop):
...
def main(op):
    ...
    if op == 0:# emittance vs qe with maxwell
        ...
        for electron_affinity in np.arange(0.1,0.8,0.1):
            ...
            ax1.annotate('(%s,%s)' % (wlscan[3,0],wlscan[3,2]),xy=(wlscan[3,0],wlscan[3,2]), textcoords='offset points')
            ...
            ax1.set_yscale('log')
            ax1.margins(0.02)        
            plt.savefig('mfp_%fea_%ftk_%d.pdf' %(mfp,electron_affinity,thickness))
            with open('ea_wl_scan.csv','a') as csv:
        ...
        #print emittance, float(len(emiss)/n)
        #print emiss
        plot(p2D,pens,p2De,None,params_exp,thickness,tempres)
        wlscan=[]
        for ep in np.arange(pestart,peend,pestep):
