...
version v5, consider electron reflect from the surface barrier
version v5 fast, using numpy matrix to fast the sim. not successful yet
version v5 alternative the e-p scattering, scattering rate is function of the energy
'''

import random
from scipy.stats.distributions import maxwell, expon
...
def diff(stept,endT,p2Di,mfp_ps,eloss,bounde,bounda,types,ea,sctk):
    ...
    if types==1:
        ...
        se,st,sb,p2De=swap(bounde,bounda,0,p2De,ea,sctk)
        ...
        while t < endT:
            ...
            foldtime=5.0# if mfp is 1, then time step is 1/mean(v)/5. the factor of mfp/mean(v) to time step.
            stept=mfp/(np.sqrt(2*np.mean(p2De[:,5])*ec/me)*10**9)/foldtime  # 0.2*tau is the step for collision
            ...
            lossoverall=1  #1, the probability of e-p scattering loss energy
                        
            ranlist=np.array([ee]*int(len(p2De)*(1/foldtime)*lossoverall)+ [-ee]*int(len(p2De)*(1/foldtime)*(1-lossoverall)) +[0]*(len(p2De)-int(len(p2De)*(1/foldtime)*lossoverall)-int(len(p2De)*(1/foldtime)*(1-lossoverall))))
            ...
            for k in range(len(p2De)):           
                ...
                if p2De[k-1,5]>bandgap:
                    r=float(mfp/(eemfp(p2De[k-1,5]-bandgap)/10)/foldtime)# the probability of carrier carrier scattering. mfp/2 is average loss? remove later, /5 is match the tstep
                ...
                if p2De[k-1,5]>0:
                    ...
                    if np.maximum(happ,nor_ranlist[k-1])==1:
                        p2De[k-1,2]=p2De[k-1,4]*math.cos(q)*np.maximum(happ,nor_ranlist[k-1])
                        p2De[k-1,3]=p2De[k-1,4]*math.sin(q)*np.maximum(happ,nor_ranlist[k-1])
                   
            
            se,st,sb,p2De=swap(bounde,bounda,0,p2De,ee,ea,sctk)    
...
def swap(be,ba,bb,mat,el,ea,sctk):
    assert ba > be
    matbackup=copy.copy(mat)
    mat[:,5]=mat[:,5]-ea-sctk+el
    mat[:,4]=np.sqrt(2*np.abs(mat[:,5])*ec/me)*10**9
    reflectind=(mat[:,0]<=be) & (mat[:,5]>=0) &(np.abs(mat[:,4])<np.abs(mat[:,3]))
    #print (mat[reflectind])
    mat[reflectind,0]=-mat[reflectind,0]# reverse the z position
    mat[reflectind,2]=-mat[reflectind,2]#reverse the z velocity
  

    emissind=(mat[:,0]<=be) & (mat[:,5]>=0) &(np.abs(mat[:,4])>=np.abs(mat[:,3]))
    #print ('mat',mat,'matbackup',matbackup)
    #print('emissind',mat[emissind])
    mat[:,5]=matbackup[:,5]
    mat[:,4]=matbackup[:,4]
    
    #print (mat,'matbackup',matbackup)
    '''
    emissind=(mat[:,0]<=be) & (mat[:,5]>=ea+sctk)
    mat[emissind,5]=mat[emissind,5]-ea-sctk
    
    mat[emissind,4]=np.sqrt(2*mat[emissind,5]*ec/me)*10**9
    print ('emissind', mat[emissind],emissind)
    reflectind=np.abs(mat[emissind,4])<np.abs(mat[emissind,3])
    print (len(reflectind),mat[reflectind,:])
    mat[reflectind,0]=-mat[reflectind,0]# reverse the z position
    mat[reflectind,2]=-mat[reflectind,2]#reverse the z velocity
    print (mat[reflectind,:],'  ',reflectind)
    mat[emissind,5]=mat[emissind,5]+ea+sctk# compensate the barrier when reflect
    emissind=mat[:,0]<=be
    '''
    surtrapind=(mat[:,0]<=be) & (mat[:,5]<ea+sctk)
    
    backind=mat[:,0]>=ba
    loweind=mat[:,5]<=bb
    #print ('surtrapind',mat[surtrapind])
    swape=mat[emissind | surtrapind,:]
    #print(swape.size,mat[emissind].size,mat[surtrapind].size)
    swape[:,5]=swape[:,5]+el
    swapt=mat[(~emissind)  &(~backind)&(~surtrapind)& loweind,:]
    swapb=mat[backind,:]
    swapr=mat[(~emissind) &(~surtrapind) &(~loweind) & (~backind),:]
    #print swapt.shape
...
def emission(mat,ea,sctk):
    ...
    emissions=mat[matind]
    #print(emissions)
    emissions[:,5]=emissions[:,5]-ea-sctk
    
    emissions[:,4]=np.sqrt(2*emissions[:,5]*ec/me)*10**9
    #mat2ind=np.abs(mat2[:,4])> np.abs(mat2[:,3])
    #print mat2ind
    #print(emissions[:,4]**2<=emissions[:,3]**2,'\n', emissions)
    emissions[:,2]=np.sqrt(emissions[:,4]**2-emissions[:,3]**2)
    surface_trap=len(mat)-len(emissions)
    '''
    ...
    '''
    Temittance=np.sqrt(np.mean((emissions[:,1]*10**-9)**2))*np.sqrt(np.mean((emissions[:,3]*10**-9)**2))/(3*10**8)  #sigma_x*sqrt(<v_x**2>)/c
    ...
    return emissions, Temittance,surface_trap
...
def emittancescanplot(wlscan,start,stop,ea,thickness,mfp,expdata,finame):
    with open(finame+'emittance_ea_wl_scan_mfp_%.3ftk_%dbg_%.3f.csv' %(mfp,thickness,bandgap),'ab') as csv:
    ...
    ax2.set_yscale('log')
    ax2.margins(0.02)        
    plt.savefig(finame+'emittance_ea_%.3fmfp_%.3ftk_%dbg_%.3f.pdf' %(ea,mfp,thickness,bandgap))
    

def electronlossscanplot(wlscan,start,stop, pestep,mfp,electron_affinity,thickness,ee,eh,expdata,finame):
    with open(finame+'ea_wl_scan_mfp_%.3fee_%deh_%d.csv' %(mfp,ee,eh),'ab') as csv:
        np.savetxt(csv,wlscan,fmt='%.8e',delimiter=',')
    ...
    ax1.set_yscale('log')
    ax1.margins(0.02)        
    plt.savefig(finame+'mfp_%.3fea_%.3ftk_%dee_%deh_%d.pdf' %(mfp,electron_affinity,thickness,ee,eh))
...
def main(op):
    ...
    dosdata = genfromtxt('K2CsSb DOS12.csv',delimiter=',')  #DOS16: bandgap:1.64eV,  DOS12: bandgap:1.23eV
    expdata= genfromtxt('cooling_exp.csv',delimiter=',')
    electron_affinity=0.55  #0.4
    thickness=50
    n=100000#100000
    #pestart=1.4
    peend=4.7
    ...
    if op == 0:# emittance vs qe with maxwell
        ...
        for electron_affinity in np.arange(0.1,0.8,0.1):
            ...
            for ep in np.arange(start,peend,pestep):
                emisst,emittancet, surtrapt=emission(diff(tstep,tend,MBdist(n,ep,thickness)[1],mfp,eloss,emisposition,thickness,3,electron_affinity,schottky)[0],electron_affinity,schottky)
        ...
        #p2D: initial energy distribution, pens: initial depth distribution, thickness: sample thickness, para and params_exp are fitting parameters
        esurf,trap,back,p2De,tempres=diff(tstep,tend,p2D,mfp,eloss,emisposition,thickness,3,electron_affinity,schottky) # diff(stept,endT,p2Di,mfp_ps,bounde,bounda,types,electron_affinity,schottky):
        # time step, time end, inital beam, mfp, emission surface, thickness, calculation type
        # esurf: reach to surface, trap: trap in body, back: get to back, p2De: all the particulates information
        emiss,emittance,surtrap=emission(esurf,electron_affinity,schottky)  # beam reach to surface, electron affinity, schottky
        #emiss: emit out beam, emittance:rms nor thermal emittance, surtrap: trap at surface
        ...
        '''
        for electron_affinity in np.arange(0.1,0.7,0.1):
            wlscan=[]
            start=bandgap1+electron_affinity+0.1
            for ep in np.arange(start,peend,pestep):
                ppd=DosDist(n,ep,thickness,dosdata)[1]
                es,tr,ba,p2dee,tempres=diff(tstep,tend,ppd,mfp,eloss,emisposition,thickness,3,electron_affinity,schottky)
        ...
        for electron_affinity in np.arange(0.4,0.5,0.02):
            wlscan=[]
            start=bandgap1+electron_affinity+0.1
            
            for ep in np.arange(start,peend,pestep):
                emisst,emittancet, surtrapt=emission(diff(tstep,tend,DosDist(n,ep,thickness,dosdata)[1],mfp,eloss,emisposition,thickness,3,electron_affinity,schottky)[0],electron_affinity,schottky)
                qe=len(emisst)*100/float(n)
                print (ep, '    ',emittancet,'   ', qe,'%' )
        ...
        data_ene,p2D,pens,params_exp=DosDist(n,petest,thickness,dosdata)
        #p2D: initial energy distribution, pens: initial depth distribution, thickness: sample thickness, para and params_exp are fitting parameters
        esurf,trap,back,p2De,tempres=diff(tstep,tend,p2D,mfp,eloss,emisposition,thickness,3,electron_affinity,schottky) # diff(stept,endT,p2Di,mfp_ps,bounde,bounda,types):
        # time step, time end, inital beam, mfp, emission surface, thickness, calculation type
        # esurf: reach to surface, trap: trap in body, back: get to back, p2De: all the particulates information
        emiss,emittance,surtrap=emission(esurf,electron_affinity,schottky)  # beam reach to surface, electron affinity, schottky
        #emiss: emit out beam, emittance:rms nor thermal emittance, surtrap: trap at surface
