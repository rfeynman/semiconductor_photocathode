...
def diff(stept,endT,p2Di,mfp_ps,eloss,bounde,bounda,types):
    ...
    if types==1:
        ...
        while t < endT:
            q=random.uniform(0,2*math.pi)
            ee=np.abs((eloss/5)*np.random.randn()+eloss )  # random energy loss  2.5 * np.random.randn(2, 4) + 3// sigma * np.random.randn(...) + mu
            ...
            p2D_trap.extend(st.tolist())
            p2D_back.extend(sb.tolist())
            print len(p2De),'  ',len(se),'   ',len(st),'    ',len(sb)
...
def main(op):
    ...
    if op == 0:# emittance vs qe with maxwell
        ...
        for ep in np.arange(pestart,peend,pestep):
            ...
            '''
            print ep, '    ',emittancet,'   ', qe,'%' 
            wlscan.append([ep, emittancet,qe,labs,backd,trapin,trapsur])
