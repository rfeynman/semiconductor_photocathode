...
bandgap=1.23   #1.23（DOS12） or 1.64(DOS16)
...
def emission(mat,ea,sctk):
    ...
    '''
    Temittance=np.sqrt(np.mean((emission[:,1]*10**-9)**2))*np.sqrt(np.mean((emission[:,3]*10**-9)**2))*np.sqrt(eff)/(3*10**8)  #sigma_x*sqrt(<v_x**2>)/c
...
def plot(p2D,pens,p2De,para,params_exp,thickness,tempres,petest,electron_affinity,finame):
    ...
    with open('thickness_%d_smooth_%dcsv' %(thickness,smooth),'ab') as csv:
...
def emittancescanplot(wlscan,start,stop,ea,thickness,mfp,expdata,finame):
    with open(finame+'emittance_ea_wl_scan_mfp_%.3ftk_%dbg_%.3feff_%.4f.csv' %(mfp,thickness,bandgap,eff),'ab') as csv:
    ...
    ax2.set_yscale('log')
    ax2.margins(0.02)        
    plt.savefig(finame+'emittance_ea_%.3fmfp_%.3ftk_%dbg_%.3feff_%.4f.pdf' %(ea,mfp,thickness,bandgap,eff))
