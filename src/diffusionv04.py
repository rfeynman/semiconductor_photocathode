...
def diff(stept,endT,p2Di,mfp_ps,eloss,bounde,bounda,types):
    ...
    if types==1:
        ...
        while t < endT:
            ...
            stept=mfp/(np.sqrt(2*np.mean(p2De[:,5])*ec/me)*10**9)/5  # 0.2*tau is the step for collision
            ...
            ranlist=np.array([0]*int(len(p2De)*0.8)+[ee]*(len(p2De)-int(len(p2De)*0.8)))
            ...
            for k in range(len(p2De)):           
                ...
                if p2De[k-1,5]>bandgap:
                    r=float(mfp/(eemfp(p2De[k-1,5]-bandgap)/10)/10)# mfp/2 is average loss, /5 is match the tstep
                    ...
                    if r<=1:
                        ...
                        p2De[k-1,5]=p2De[k-1,5]-random.uniform(bandgap,p2De[k-1,5])*happ*selc[0] #e-e scattering
...
def main(op):
    ...
    mfp=4  #5
    emisposition=0
    eloss=0.027#0.01  mfp5nm,0.027eV
    ee=1
    eh=1
    ...
    if op == 0:# emittance vs qe with maxwell
        ...
        # for electron_affinity in np.arange(0.3,0.6,0.1):
        for thickness in np.arange(50,100,10):
            wlscan=[]
            start=bandgap1+electron_affinity+0.1
    ...
    elif op==4:   # emittance vs qe with Dos, wavelength scan
