...
version v5 john's suggestion, e-e dominate, reflection from surface
'''
        ...
        while t < endT:
            ...
            for k in range(len(p2De)):           
                ...
                if p2De[k-1,5]>bandgap:
                    ...
                    selc=[1,0] # the ratio of e-e and e-h scattering
                    #print int(r*100)
                    if r<=1:
...
def emission(mat):
    ...
    '''
    Temittance=np.sqrt(np.mean((mat[:,1]*10**-9)**2))*np.sqrt(np.mean((mat[:,3]*10**-9)**2))/(3*10**8)  #sigma_x*sqrt(<v_x**2>)/c
...
def emittancescanplot(wlscan,start,stop,ea,thickness,mfp,expdata):
    with open('emittance_ea_wl_scan_mfp_%.3ftk_%dbg_%.3f.csv' %(mfp,thickness,bandgap),'a') as csv:
    ...
    ax2.set_yscale('log')
    ax2.margins(0.02)        
    plt.savefig('emittance_ea_%.3fmfp_%.3ftk_%dbg_%.3f.pdf' %(ea,mfp,thickness,bandgap))
