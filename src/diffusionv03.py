...
def diff(stept,endT,p2Di,mfp_ps,eloss,bounde,bounda,types):
    ...
    if types==1:
        ...
        while t < endT:
            ...
            ee=np.abs((eloss/3)*np.random.randn()+eloss )  # random energy loss  2.5 * np.random.randn(2, 4) + 3// sigma * np.random.randn(...) + mu
            mfp=np.abs((mfp_ps/3)*np.random.randn()+mfp_ps)   #random mfp
            
            p2De[:,4]=np.sqrt(2*p2De[:,5]*ec/me)*10**9
            ...
            for k in range(len(p2De)):           
                q=random.uniform(0,2*math.pi)
                p2De[k-1,2]=p2De[k-1,4]*math.cos(q)
                p2De[k-1,3]=p2De[k-1,4]*math.sin(q)
                
                # start a e-e scattering   stept     ,eemfp, or e-h scattering
                if p2De[k-1,5]>bandgap:
                    r=float(mfp/(eemfp(p2De[k-1,5]-bandgap)/10)/2)
                    
                    sec1=random.randint(0,1)
                    selc=[sec1,1-sec1]
                    #print int(r*100)
                    if r<=1:
                        ...
                        p2De[k-1,5]=p2De[k-1,5]-random.uniform(bandgap,p2De[k-1,5])*random.choice(ranlist)*selc[0] #e-e scattering
                        p2De[k-1,5]=p2De[k-1,5]- random.uniform(0,1.2)*random.choice(ranlist)*selc[1] #e-h scattering
                     
                    else:
                        p2De[k-1,5]=p2De[k-1,5]-np.minimum(int(r),int(p2De[k-1,5]/bandgap))*random.uniform(bandgap,p2De[k-1,5])*selc[0]#e-e scattering *int(mfp/(eemfp(p2De[k-1,5]-bandgap)/10))
                        p2De[k-1,5]=p2De[k-1,5]-np.minimum(int(r),int(p2De[k-1,5]/bandgap))*random.uniform(0,1.2)*selc[1]#e-h scattering
...
def emission(mat,ea,sctk):
    ...
    mat2[:,4]=np.sqrt(2*mat2[:,5]*ec/me)*10**9
    ...
    '''
    Temittance=np.sqrt(np.mean((emission[:,1]*10**-9)**2))*np.sqrt(np.mean((emission[:,3]*10**-9)**2))/(3*10**8)  #sigma_x*sqrt(<v_x**2>)/c
...
def plot(p2D,pens,p2De,para,params_exp,thickness,tempres,petest,electron_affinity):
    ...
    plt.savefig('full_tk_%dpe_%.3fea_%.3f.pdf'%(thickness,petest,electron_affinity))
    plt.show()
    return

def emittancescanplot(wlscan,start,stop,ea,thickness,mfp,expdata):
    with open('emittance_ea_wl_scan_mfp_%.3ftk_%d.csv' %(mfp,thickness),'a') as csv:
    ...
    ax2.set_yscale('log')
    ax2.margins(0.02)        
    plt.savefig('emittance_ea_%.3fmfp_%.3ftk_%d.pdf' %(ea,mfp,thickness))
    

def electronlossscanplot(wlscan,start,stop, pestep,mfp,electron_affinity,thickness,ee,eh,expdata):
    with open('ea_wl_scan_mfp_%.3fee_%deh_%d.csv' %(mfp,ee,eh),'a') as csv:
        np.savetxt(csv,wlscan,fmt='%.8e',delimiter=',')
    ...
    ax1.set_yscale('log')
    ax1.margins(0.02)        
    plt.savefig('mfp_%.3fea_%.3ftk_%dee_%deh_%d.pdf' %(mfp,electron_affinity,thickness,ee,eh))
...
def main(op):
    ...
    mfp=3  #5
    emisposition=0
    eloss=0.0162#0.01  mfp5nm,0.027eV
    ee=1
    eh=1
    ...
    if op == 0:# emittance vs qe with maxwell
        ...
        plot(p2D,pens,p2De,para,params_exp,thickness,tempres)
        ...
        for electron_affinity in np.arange(0.1,0.8,0.1):
            ...
            emittancescanplot(wlscan,start,peend,electron_affinity,thickness,mfp,expdata)
        print wlscan
        



    elif op==1:   # wavelength scan with electrons go
        '''
        ...
        for electron_affinity in np.arange(0.1,0.8,0.1):
            ...
            wlscan=np.array(wlscan)           
            stop=peend
            electronlossscanplot(wlscan,start,stop, pestep,mfp,electron_affinity,thickness,ee,eh,expdata)
        #plt.show()
        #print wlscan  
    ...
    elif op==2:  # emittance vs qe with Dos
        '''
        data_ene,p2D,pens,params_exp=DosDist(n,petest,thickness,dosdata)
        #p2D: initial energy distribution, pens: initial depth distribution, thickness: sample thickness, para and params_exp are fitting parameters
        esurf,trap,back,p2De,tempres=diff(tstep,tend,p2D,mfp,eloss,emisposition,thickness,3) # diff(stept,endT,p2Di,mfp_ps,bounde,bounda,types):
        ...
        for electron_affinity in np.arange(0.1,0.8,0.1):
            ...
            for ep in np.arange(start,peend,pestep):
                emisst,emittancet, surtrapt=emission(diff(tstep,tend,DosDist(n,ep,thickness,dosdata)[1],mfp,eloss,emisposition,thickness,3)[0],electron_affinity,schottky)
                qe=len(emisst)*100/float(n)
                print ep, '    ',emittancet,'   ', qe,'%' 
                wlscan.append([ep, emittancet,qe])
